...
app.controller("restaurantCtrl", function($scope, $http) {
	...
	$scope.fetchRestaurant = function() {
		$http({
			method : 'GET',
			url : 'http://localhost:8080/restuarent/getall'
	...
	$scope.saveRestaurant = function() {
		$http({
			method : 'POST',
			url : 'http://localhost:8080/restuarent/create',
...
app.controller("itemsCtrl", function($scope, $http) {
	...
	$scope.saveItem = function() {
		$http({
			method : 'POST',
			url : 'http://localhost:8080/restuarent/additem',
			headers: { 'Content-Type': 'application/json' },
			data:$scope.items
		}).success(function(data, status) {
			console.log(data);
	...
	$scope.getRestaurantDet = function() {
		$http({
			method : 'POST',
			url : 'http://localhost:8080/restuarent/findrest',
			headers: { 'Content-Type': 'application/json' },
			data:$scope.items
		}).success(function(data, status) {
			console.log(data);
